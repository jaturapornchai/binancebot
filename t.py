...
future_api = gate_api.FuturesApi(api_client)
...
def check_shot_div_signal(symbol):
    timeframe = '1h'
    limit = 1000  # จำนวนแท่งข้อมูลที่ดึงต่อครั้ง
    since = exchange.milliseconds() - 1000 * 60 * 60 * 24 * 30  # ดึงข้อมูลย้อนหลัง 30 วัน
    ...
    if divergences['bearish']:
        latest_divergence = divergences['bearish'][-1][0]
        time_since_divergence = (data.index[-1] - latest_divergence) // pd.Timedelta(minutes=60)
        if time_since_divergence < 6:
            print(f"Symbol: {symbol} - Latest bearish divergence detected at {latest_divergence}, {time_since_divergence} time frames ago.",flush=True)
            return True
    return False
    ...
    for symbol in order_symbols:
        ...
        if buy_signal:
            ...
            if volume_usd > 75000:            
                ...
                print(f"Symbol: {symbol} - Volume is too low.",flush=True)
...
def future_close_open_order():
    try:
        # Get all open orders
        open_orders = future_api.list_futures_orders(settle="usdt", status="open")
        print(f"Found {len(open_orders)} open orders.", flush=True)
        for order in open_orders:
            print(f"Cancel order {order.id}", flush=True)
            try:
                # Call the cancel order function with the correct parameters
                future_api.cancel_futures_order(order_id=order.id, settle="usdt")
            except ApiException as e:
                print(f"Error cancelling order {order.id}:", e, flush=True)
    except ApiException as e:
        print("Error fetching open orders:", e, flush=True)

def future_open_short_position(symbol, amount_usd=20):
    print(f"Open short position for {symbol} with {amount_usd} USDT", flush=True)
    try:
        # Fetch the latest ticker for the trading pair
        ticker = future_api.get_futures_ticker(settle="usdt", contract=symbol)
        current_price = float(ticker['last'])  # Get the latest price
        quantity = amount_usd / current_price  # Calculate the quantity to sell
        # Increase current_price by 0.5%
        limit_price = current_price * 1.005

        # Create a limit order for selling (shorting)
        order = {
            "contract": symbol,
            "size": -quantity,  # Negative size indicates a short position
            "price": str(limit_price),
            "tif": "gtc"  # Time in force: Good Till Cancelled
        }
        
        future_api.create_futures_order(settle="usdt", order=order)
    except GateApiException as ex:
        print(f"Gate API Exception, label: {ex.label} message: {ex.message}", flush=True)
    except ApiException as e:
        print(f"API Exception when calling FuturesApi->create_order: {e}", flush=True)
    except Exception as e:
        print(f"Unexpected error: {e}", flush=True)   

def future_get_symbol():
    try:
        # ดึงรายการสัญลักษณ์ Future ทั้งหมด
        futures = future_api.list_futures_contracts(settle="usdt")

        # แสดงรายการสัญลักษณ์
        for future in futures:
            try:
                shot_signal = check_shot_div_signal(future.name)
                if (shot_signal):
                    print(f"Symbol: {future.name} - Shot signal detected.",flush=True)
                    future_open_short_position(future.name)
            except Exception as e:
