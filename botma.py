...
def fetch_data(symbol, interval, lookback):
    ...
    frame = pd.DataFrame(client.get_historical_klines(symbol, interval, lookback + ' days ago UTC'))
...
def detect_MA_cross(short_ma, long_ma):
    """ Detect MA Cross """
...
def analyze_symbol(symbol):
    lookback = '1 month'  # Lookback period

    data = fetch_data(symbol, time_frame, lookback)
    
    short_ma = calculate_MA(data, 50)
    long_ma = calculate_MA(data, 200)
    signal = detect_MA_cross(short_ma, long_ma)
...
def place_order_future(client, symbol, direction, dollar_amount):  
    ...
    if (direction == 'long'):
        ...
        # ค้นหาราคาต่ำสุดย้อนหลัง 14 bars
        ohlcv = exchange.fetch_ohlcv(symbol, timeframe=time_frame, limit=14)
        df = pd.DataFrame(ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
        ...
        take_profit_price = enter_price + ((enter_price - stop_loss_price) * 0.75)
        ...
        enter_price = float(order['avgPrice'])
        order_id = order['orderId']
        # ค้นหาราคาสูงสุดย้อนหลัง 14 bars
        ohlcv = exchange.fetch_ohlcv(symbol, timeframe=time_frame, limit=14)
        df = pd.DataFrame(ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
        ...
        take_profit_price = enter_price - ((stop_loss_price - enter_price) * 0.75)
        take_profit_price = adjust_to_precision(take_profit_price, price_precision)
...
def findShortLong():
    ...
    dollar_amount = round((balance / 100) * leverage)
    ...
    while True:
        try:
            ...
            if tm_min == 5 or first_run == True:
                ...
                exchange_rate_thai = get_exchange_rate_thai()
                time.sleep(60)
            if tm_min % 16 == 0 and time.localtime(time.time()).tm_sec == 0:
