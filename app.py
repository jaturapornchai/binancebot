...
class GateIOShortScanner:
    ...
    def calculate_linear_regression(self, contract: str, df: pd.DataFrame) -> pd.DataFrame:
        ...
        print(f"Calculated LR for {contract}: Middle={df['middle_line'].iloc[-1]:.4f}, Upper={df['upper_line'].iloc[-1]:.4f}, Lower={df['lower_line'].iloc[-1]:.4f}", flush=True)
        return df.dropna()
    def check_trading_signal(self, df: pd.DataFrame, current_price: float) -> str:
        ...
        current = df.iloc[-1]
        previous = df.iloc[-2]
        if current['high'] >= current['upper_line'] and previous['close'] > previous['upper_line'] and current_price < current['upper_line']:
            print(f"SELL signal detected for {df.name}: C0 High={current['high']:.4f} >= Upper={current['upper_line']:.4f}, C1 Close={previous['close']:.4f} > Upper={previous['upper_line']:.4f}, Price={current_price:.4f} < Upper={current['upper_line']:.4f}", flush=True)
            return "SELL"
        return None
            ...
            for pos in positions:
                ...
                if not df.empty:
                    ...
                    if size < 0 and current_price > upper:
                        print(f"Closing SHORT position for {contract}: Price={current_price:.4f} > Upper={upper:.4f}", flush=True)
