...
class GateIOLinearRegressionTrader:
    def __init__(self):
        ...
        self.leverage, self.order_amount, self.lookback_period, self.devlen = 5, 20, 100, 2.0
    ...
    def get_futures_contracts(self) -> List[str]:
        ...
        valid_contracts = [t.contract for t in ticket if re.match(r'^\D+_USDT$', t.contract) and t.contract not in ['USDC_USDT', 'DOGS_USDT']]
    ...
    def calculate_linear_regression_channel(self, df: pd.DataFrame) -> dict:
        ...
        middle, deviations = line[-1], recent_data - line
    ...
    def check_trading_signal(self, df: pd.DataFrame, lrc_data: dict, contract: str = None) -> str:
        ...
        # เงื่อนไขใหม่ SELL: แท่งเทียนสีแดง ทับเส้น BOTTOM และราคาล่าสุดอยู่ใต้เส้น BOTTOM
            ...
            for pos in positions:
                ...
                if not df.empty:
                    ...
                    if lrc_data:
                        ...
                        if latest_price:
                            top, bottom, middle, slope = lrc_data['TOP'], lrc_data['BOTTOM'], lrc_data['MIDDLE'], lrc_data['slope']
                            ...
                            # เงื่อนไขการปิด position ใหม่
                            close_position_reason = None
                            
                            # ปิด long position ถ้า position เดิมที่เปิดอยู่เป็น long และราคาล่าสุดต่ำกว่าเส้น TOP
                            if size > 0 and latest_price < top:
                                close_position_reason = f"ราคาล่าสุด ({latest_price:.6f}) ต่ำกว่าเส้น TOP ({top:.6f})"
                            # ปิด short position ถ้า position เดิมที่เปิดอยู่เป็น short และ ราคาล่าสุด สูงกว่าเส้น BOTTOM
                            elif size < 0 and latest_price > bottom:
                                close_position_reason = f"ราคาล่าสุด ({latest_price:.6f}) สูงกว่าเส้น BOTTOM ({bottom:.6f})"
                    ...
                    for i, contract in enumerate(contracts, 1):
                        ...
                        if not df.empty:
                            ...
                            if lrc_data:
                                ...
                                middle, slope = lrc_data['MIDDLE'], lrc_data['slope']
                                ...
                                if signal == "BUY":
                                    ...
                                    existing_pos = self.check_existing_position(contract)
                                    if existing_pos and float(existing_pos['size']) > 0:
                                        # มี LONG position ให้ปิดก่อนแล้วเปิด SHORT
