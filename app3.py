...
def future_create_position(symbol, side):
    ...
    try:
        ...
        if side == 'BUY' or side == 'SELL':
            ...
            getfuture = client.futures_position_information(symbol=symbol)
            for item in getfuture:
                ...
                if close_position_amount != 0:
                    ...
                    client.futures_create_order(
                        symbol=symbol, side=close_side, type='MARKET', quantity=abs(close_position_amount), timestamp=timestamp, recvWindow=myRecvWindow
                    )
                    time.sleep(2)
        ...
        if side == 'BUY':
            ...
            time.sleep(2)
            create_position_stop_loss_take_profit(symbol,side, current_price,quantity)
        elif side == 'SELL':
            # สร้าง order ขาย
            client.futures_create_order(
...
def create_position_stop_loss_take_profit(symbol, side,current_price,quantity):
    return
    try:
        # ซิงค์เวลาและดึง offset จากฟังก์ชัน sync_time_with_server
        offset = sync_time_with_server(client)
        ...
        # ลบ stop loss และ take profit ที่มีอยู่ก่อนหน้า
        ...
        heighs = [float(kline[2]) for kline in klines]
        if side == 'BUY':
            stop_loss = min(lows)
        ...
        print(f"Error creating stop loss and take profit for {symbol}: {e}", flush=True)        
            ...
            try:
                ...
                if position_amount != 0:                
                    ...
                    if side == 'LONG':
                        ...
                        stop_loss = math.floor(stop_loss / price_step_size(symbol)) * price_step_size(symbol)
                        # take profit = ช่องว่างระหว่าง ราคาปัจจุบัน - stop loss คูณ 1.25 + stop loss
                        take_profit = ((position_entry_price - stop_loss) * 1.25) + position_entry_price
                        ...
                        stop_loss = math.ceil(stop_loss / price_step_size(symbol)) * price_step_size(symbol)
                        # take profit = ช่องว่างระหว่าง ราคาปัจจุบัน - stop loss หาร 2 + stop loss * 1.25
                        take_profit = position_entry_price - ((stop_loss - position_entry_price) * 1.25) 
                        if take_profit > position_entry_price:
                            take_profit = position_entry_price
                        take_profit = math.floor(take_profit / price_step_size(symbol)) * price_step_size(symbol)
                        position_amount = float(position['positionAmt'])
...
def supertrend(data, period=10, multiplier=3.0):
    ...
    supertrend = pd.Series([True] * len(data), index=data.index)
    ...
    for i in range(1, len(data)):
        if data['close'].iloc[i] > upperband.iloc[i-1]:
            ...
            if supertrend.iloc[i] == True and lowerband.iloc[i] < lowerband.iloc[i-1]:
                lowerband.iloc[i] = lowerband.iloc[i-1]
            if supertrend.iloc[i] == False and upperband.iloc[i] > upperband.iloc[i-1]:
...
def check_signal(symbol, interval, period=10, multiplier=3.0):
    df = get_binance_data(symbol, interval)
    df['supertrend'] = supertrend(df, period, multiplier)
